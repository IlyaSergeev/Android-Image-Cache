 ...
 * 
 * <p>
 * To use, pass in a {@link ListAdapter} that generates {@link ImageView}s in
 * the layout hierarchy of getView(). ImageViews are searched for using the IDs
 * specified in {@code imageViewIDs}. When found,
 * {@link ImageView#getTag(R.id.ic__uri)} is called and should return a
 * {@link Uri} referencing a local or remote image. See
 * {@link ImageCache#loadImage(int, Uri, int, int)} for details on the types of
 * URIs and images supported.
 * </p>
 * 
 * @author <a href="mailto:spomeroy@mit.edu">Steve Pomeroy</a>
 * @author <a href="mailto:master.hensh@gmail.com">ilyasergeev add some functionality
 */
public class ImageLoaderAdapter extends AdapterWrapper implements ImageCache.OnImageLoadListener
{
	...
	private class LoadResult
	{
		...
		LoadResult(int id, Uri imageUri, Drawable image)
		{
			this.id = id;
	...
	private static class LoadingHanler extends Handler
	{
		SoftReference<ImageLoaderAdapter> ownerSoftRef;

		LoadingHanler(ImageLoaderAdapter owner)
		{
			ownerSoftRef = new SoftReference<ImageLoaderAdapter>(owner);
		}
		...
		public void handleMessage(Message msg)
		{
			...
			{
				if (ownerSoftRef == null)
				{
					return;
				}
				ImageLoaderAdapter owner = ownerSoftRef.get();
				if (owner == null)
				{
					return;
				}
				...
				if (owner.isEnableUpdate())
				{
					owner.onImageLoaded(result.id, result.imageUri, result.image);
				}
				else
				{
					synchronized (owner.notClaimedResults)
					{
						owner.notClaimedResults.add(result);
					}
				}
			}
			else
	...
	private final List<LoadResult> notClaimedResults = new ArrayList<LoadResult>();
	private final LoadingHanler mHandler;
	 ...
	 */
	public ImageLoaderAdapter(Context context, ListAdapter wrapped, ImageCache cache, int[] imageViewIDs, int defaultWidth, int defaultHeight, int unit)
	{
	 ...
	 */
	public ImageLoaderAdapter(Context context, ListAdapter wrapped, ImageCache cache, int[] imageViewIDs, int defaultWidth, int defaultHeight, int unit, boolean autosize)
	{
		super(wrapped);

		mHandler = new LoadingHanler(this);
	...
	public void onImageLoaded(int id, Uri imageUri, Drawable image)
	{
		...
		{
			synchronized (notClaimedResults)
			{
				notClaimedResults.add(new LoadResult(id, imageUri, image));
			}
		}
	}
	...
	public void setEnableUpdate(boolean enableUpdate)
	{
		...
		if (enableUpdate)
		{
			synchronized (notClaimedResults)
			{
				for (LoadResult result : notClaimedResults)
				{
					mHandler.sendMessage(mHandler.obtainMessage(MSG_LOADED, result));
				}
				notClaimedResults.clear();
			}
		}
	}
